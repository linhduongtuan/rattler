...
//!  - [none()](DistinctRange::none): the empty set
//!  - [any()](DistinctRange::any): the set of all possible versions
//!  - [exact(v)](DistinctRange::exact): the set containing only the version v
//!  - [higher_than(v)](DistinctRange::higher_than): the set defined by `v <= versions`
//!  - [strictly_lower_than(v)](DistinctRange::strictly_lower_than): the set defined by `versions < v`
//!  - [between(v1, v2)](DistinctRange::between): the set defined by `v1 <= versions < v2`
...
pub struct DistinctRange<V: Version> {
...
impl<V: Version> DistinctRange<V> {
...
impl<V: Version> DistinctRange<V> {
    ...
    fn negate_segments(start: V, segments: &[Interval<V>]) -> DistinctRange<V> {
...
impl<V: Version> fmt::Display for DistinctRange<V> {
...
//     use super::*;
//
//     pub fn strategy() -> impl Strategy<Value = DistinctRange<NumberVersion>> {
...
//             assert_eq!(DistinctRange::any().intersection(&range), range);
//         }
//
//         #[test]
//         fn intersection_with_none_is_none(range in strategy()) {
//             assert_eq!(DistinctRange::none().intersection(&range), DistinctRange::none());
//         }
//
//         #[test]
...
//             assert_eq!(range.negate().intersection(&range), DistinctRange::none());
//         }
//
//         #[test]
...
//             assert_eq!(range.negate().union(&range), DistinctRange::any());
//         }
//
//         #[test]
...
//             assert!(DistinctRange::exact(version).contains(&version));
//         }
//
//         #[test]
...
//             assert_eq!(range.contains(&version), range.intersection(&DistinctRange::exact(version)) != DistinctRange::none());
//         }
//     }
// }
